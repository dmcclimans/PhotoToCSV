...
// Versions
// 2023-03-26 Initial version downloaded
// 2023-03-26 Add conditional compile to work under .net Framework or .net 6.0 (C# 10 with nullable types).
//            Add static Caption property and methods that don't require a caption parameter.
// 2023-03-30 Remove Show methods without a parent/owner window.
...
namespace CenteredMessageBox
{
    ...
    public static class MessageBoxEx
    {
        ...
        // Set the caption to use on all calls if not specified.
        public static string Caption { get; set; } = "";

        #region Show
#if false
        // The Show() methods without a parent (owner) window are disabled with #if false. This 
        // is to prevent accidental use. If you intentionally want a Show() without owner, use the
        // regular Windows.Forms.MessageBox method.
...
#if NET6_0_OR_GREATER
            object? obj = Marshal.PtrToStructure(lParam, typeof(CWPRETSTRUCT)) ?? throw new NullReferenceException();
#else
            object obj = Marshal.PtrToStructure(lParam, typeof(CWPRETSTRUCT)) ?? throw new NullReferenceException();
#endif
            CWPRETSTRUCT msg = (CWPRETSTRUCT)obj;
            IntPtr hook = _hHook;
        ...
        private static void CenterWindow(IntPtr hChildWnd)
        {
            if (_owner == null)
            {   
                // This cannot happen, but checking for it avoids null object warnings from compiler.
            ...
            {
                // -32000, -32000 is where minimized windows are parked.
                _ = NativeMethods.UnhookWindowsHookEx(_hHook);
                return;
            }
